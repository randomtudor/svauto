...
public abstract class WebUiPage {
  ...
  public WebUiPage(final WebDriver driver) {
    this.driver = driver;
  ...
  public void waitForElementToAppear(final By element) {
    waitFor(ExpectedConditions.visibilityOfElementLocated(element), TIMEOUT);
  }

  public void waitForElementToDisappear(final By element) {
  ...
  protected void verify(final boolean executeIsValid) {
      ...
      if (browserFinishedLoading) {
        ...
        if (isCurrent()) {
          if (!executeIsValid) {
            LOG.info(
                String.format(
                    "Page %s is current now. Skipping check whether it's valid", pageName));
          } else {
            LOG.info(
                String.format("Page %s is current now. Checking whether it's valid ...", pageName));
            if (!isValid()) {
    ...
    throw new PageNotCurrentException(
        pageName
            + " page did not load within "
            + waitTime
            + " seconds (the exact wait time was: "
            + totalWaitTime
            + "ms)");
  }
   ...
   *
   * <p>NOTE: This method should NOT CONTAIN ANY WAITS. It should simply check for the minimum
   * amount of elements that uniquely identify the page / state in the browser to be present
   *
   * <p>This method should also not perform any type of document ready state checks, or spinners not
   * being present etc. This is done from within {@link #isBrowserDoneLoading()}
   *
   * <p>It will be called from within {@link #verify()} inside of a time-based loop after the page
   ...
   *
   * <p>NOTE: This method should NOT CONTAIN ANY WAITS. It should simply check whether all elements
   * currently displayed on the page are valid / correct
   *
   * <p>This method will be called after {@link #isCurrent()} returned true, so you can assume the
  ...
  private boolean isBrowserDoneLoading() {
    return Objects.equals(
        Utilities.executeScript(this.driver, DOCUMENT_READY_STATE_GET),
